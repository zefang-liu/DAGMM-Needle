...
/// BEGIN YOUR SOLUTION
__device__ size_t ConvertGidToIndex(
  size_t gid, CudaVec shape, CudaVec strides, size_t offset) {
  CudaVec new_strides;
  new_strides.size = shape.size;

  size_t strid = 1;
  for (size_t dim = shape.size - 1; dim < shape.size; dim--) {
    new_strides.data[dim] = strid;
    strid *= shape.data[dim];
  }

  size_t index = offset;
  size_t new_gid = gid;
  for (size_t dim = 0; dim < shape.size; dim++) {
    index += (new_gid / new_strides.data[dim]) * strides.data[dim];
    new_gid = new_gid % new_strides.data[dim];
  }
...
__global__ void CompactKernel(const scalar_t* a, scalar_t* out, size_t size, CudaVec shape,
  ...
  /// BEGIN YOUR SOLUTION
  if (gid < size) {
    size_t index = ConvertGidToIndex(gid, shape, strides, offset);
    out[gid] = a[index];
  }
  /// END YOUR SOLUTION
}
...
__global__ void EwiseSetitemKernel(
  const scalar_t* a, scalar_t* out, size_t size,
...
__global__ void ScalarSetitemKernel(
  const scalar_t val, scalar_t* out, size_t size,
  CudaVec shape, CudaVec strides, size_t offset) {
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void ReduceMaxKernel(
  const scalar_t* a, scalar_t* out,
  size_t size, size_t reduce_size) {
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;

  if (gid < size) {
    scalar_t maxNum = a[gid * reduce_size];
    for (size_t i = gid * reduce_size; i < (gid + 1) * reduce_size; i++) {
      maxNum = fmax(maxNum, a[i]);
    }
    out[gid] = maxNum;
  }
}
