...
class NDArray:
    ...
    def reshape(self, new_shape):
        ...
        Returns:
            NDArray : reshaped array; this will point to the same memory as the
            original array.
        """

        ### BEGIN YOUR SOLUTION
        return NDArray.make(
            shape=new_shape, strides=NDArray.compact_strides(new_shape),
    ...
    def permute(self, new_axes):
        ...
        for new_axis in new_axes:
            new_shape.append(self._shape[new_axis])
            new_strides.append(self._strides[new_axis])
    ...
    def broadcast_to(self, new_shape):
        ...
        for dim in range(len(self._shape)):
            if self._shape[dim] != 1:
                assert new_shape[dim] == self._shape[dim]
    ...
    def __getitem__(self, idxs):
        ...
        for dim, idx in enumerate(idxs):
            new_shape.append((idx.stop - idx.start + idx.step - 1) // idx.step)
            new_strides.append(self._strides[dim] * idx.step)
            offset += self._strides[dim] * idx.start
    ...
    def flip(self, axes):
        ...
        return NDArray.make(
            shape=self._shape, strides=new_strides,
            device=self._device, handle=self._handle,
    ...
    def pad(self, axes):
        ...
        axes = ( (0, 0), (1, 1), (0, 0)) pads the middle axis with a 0 
        ...
        for axis, padding in enumerate(axes):
            ...
            new_shape.append(
                self._shape[axis] + left_padding + right_padding)
            slices.append(
                slice(left_padding, left_padding + self._shape[axis], 1))

        slices = tuple(slices)
        out = full(
            shape=new_shape, fill_value=0, 
