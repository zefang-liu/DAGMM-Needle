...
def compute_gradient_of_variables(output_tensor, out_grad):
    ...
    for node in reverse_topo_order:
        ...
        for grad in node_to_output_grads_list[node][1:]:
            node.grad += grad

        if node.op is not None:
            gradients = node.op.gradient(node.grad, node)
            if type(gradients) is not tuple:
                gradients = (gradients,)

            for input_node, gradient in zip(node.inputs, gradients):
                if input_node not in node_to_output_grads_list:
                    node_to_output_grads_list[input_node] = []
                node_to_output_grads_list[input_node].append(gradient)
    ### END YOUR SOLUTION
